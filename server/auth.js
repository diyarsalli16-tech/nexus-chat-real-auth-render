...
export async function requireAuth(req, res, next) {
  try {
    ...
    const result = await query(`SELECT id, username, avatar, status, bio, created_at FROM users WHERE id=$1`, [payload.id]);
...
export async function socketAuth(socket, next) {
  try {
    ...
    const payload = jwt.verify(token, secret());
    const result = await query(`SELECT id, username, avatar, status, bio FROM users WHERE id=$1`, [payload.id]);
